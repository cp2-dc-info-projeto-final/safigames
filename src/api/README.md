...
```sql
CREATE USER safigames WITH ENCRYPTED PASSWORD 'safigames';
GRANT ALL ON SCHEMA public TO safigames;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO safigames;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO safigames;
```
...
```bash
 psql -h localhost  -d safigames -U safigames -W -f db/create.sql
```
